...
from eval import launch_plan_eval, launch_unroll_eval
...
def run(
    ...
    if folder is None:
        if cfg.meta.get("model_folder"):    # None 
            folder = Path(cfg.meta.model_folder)
            folder_name = folder.name 
            exp_name = folder_name.rsplit("_seed", 1)[0]
        else:
            sweep_name = get_default_dev_name()             # "dev_%Y-%m-%d_%H-%M"
            exp_name = get_exp_name("ac_video_jepa",cfg)    # impala_cov_coeff8_std_coeff....
            folder = get_unified_experiment_dir(            # create a dir
                example_name="ac_video_jepa",
                sweep_name=sweep_name,
                exp_name=exp_name,
                seed=cfg.meta.seed
            )
            # folder : checkpoints/ac_video_jepa/swee_name{dev...}/exp_name{impala_cov....}
    else:
        folder = Path(folder)
        folder_name = folder.name
        exp_name = folder_name.rsplit("_seed", 1)[0]
    ...
    # --SETUP 
    setup_device("auto")
    setup_seed(cfg.meta.seed)   # random, np, torch, cuda 
    ...
    if enable_eval:
        ...
        with open(cfg.eval.plan_cfg_path, "r") as f:    #ends up with cfgs/planning_mppi.yaml
            plan_cfg = yaml.load(f, Loader=yaml.FullLoader) # safe_load is fine, no need to use load
        plan_cfg["logging"] = copy.deepcopy(dict(cfg.logging))
        with open(cfg.eval.eval_cfg_path,"r") as f:     #ends up with cfgs/eval.yaml
    ...
    steps_per_epoch = data_config.size // data_config.batch_size    # data_config.size is train samples
    ...
    jepa_optimizer = AdamW(jepa.parameters(),lr = cfg.optim.lr,weight_decay=cfg.optim.get("weight_decay", 1e-6))
    ...
    if cfg.meta.load_model:
        ...
        ckpt_info = load_checkpoint(checkpoint_path, jepa, jepa_optimizer, jepa_scheduler, device=device)
    ...
    # --EVAL ONLY MODE
    if cfg.meta.get("eval_only_mode", False):   # False 
        if not enable_eval:
    ...
    # -- TRAINING LOOP 
    for epoch in range(start_epoch, cfg.optim.epochs):  # (0, 12)
        epoch_start_time = time()
        pbar = tqdm(
            enumerate(loader),
            total=len(loader),  # 260 
        ...
        for idx, (x, a, loc, _, _) in pbar:
            ...
            if cfg.optim.get("grad_clip_enc") and cfg.optim.get("grad_clip_pred"):  # 2.0 and 2.0 
            ...
            with autocast(device.type, enabled=use_amp, dtype=dtype):
                xy_loss = xy_prober(
                    observations= x[:, :, :1],  # single frame per clip (decorrelated) and it's enough to train prober.
                    targets=loc[:, :, :1]
                )
                xy_loss = loader.dataset.normalizer.unnormalize_mse(xy_loss)    # mse =  mse_norm * (std**2)
                total_loss += xy_loss
